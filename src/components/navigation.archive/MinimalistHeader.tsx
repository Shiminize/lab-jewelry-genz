...
export function MinimalistHeader({ className }: MinimalistHeaderProps) {
  ...
  return (
    <header className={cn(
      'sticky top-0 z-50 w-full bg-background/95 backdrop-blur border-b border-aurora-nav-border',
      className
    )}>
      ...
      <div className="bg-gradient-to-r from-cta/5 to-accent/5 border-b border-cta/10">
        <div className="container mx-auto px-4 py-2">
          <div className="flex items-center justify-center space-x-6 text-sm">
            ...
            <div className="text-aurora-nav-muted bg-background">70% more affordable</div>
            <div className="text-aurora-nav-muted bg-background">90% more sustainable</div>
            <Link
              href="/learn-more"
      ...
      <div className="lg:hidden border-t border-aurora-nav-border bg-background">
          ...
          <div className="grid grid-cols-2 gap-4">
            {minimalistNavigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="flex items-center justify-between p-3 rounded-lg border border-aurora-nav-border hover:border-cta/20 hover:bg-cta/5 transition-all"
              >
                <div>
                  <div className="font-medium text-foreground bg-background">{item.name}</div>
                  <div className="text-xs text-aurora-nav-muted bg-background">{item.description}</div>
                </div>
                {item.badge && (
                  <span className="text-xs bg-cta/10 text-cta px-2 py-0.5 rounded-full font-medium">
                    {item.badge}
