            ...
            {mainNavigation.map((category) => (
              ...
              >
                ...
                {hoveredCategory === category.id && (
                  <div 
                    className="absolute top-full left-1/2 transform -translate-x-1/2 w-screen max-w-4xl bg-background border border-border shadow-xl rounded-lg mt-2 p-8 z-50 animate-fade-in"
                    onMouseEnter={() => setHoveredCategory(category.id)}
                    onMouseLeave={() => setHoveredCategory(null)}
                  >
                    <div className="mb-6 pb-4 border-b border-border">
                      <h3 className="font-headline text-xl font-semibold text-foreground mb-2">{category.label}</h3>
                      <p className="font-body text-sm text-aurora-nav-muted max-w-2xl">{category.description}</p>
                    </div>
            ...
            <div className="p-6 border-b border-border bg-muted">
              <div className="relative">
                ...
                <input
                  type="search"
                  placeholder="Search luxury jewelry..."
                  className="font-body w-full pl-10 pr-4 py-3 bg-background border border-border rounded-lg focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 text-foreground placeholder:text-foreground opacity-70"
                />
              </div>
            </div>
