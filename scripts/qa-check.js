...
class QAChecker {
  constructor() {
    this.results = {
      ...
      playwright: { passed: false, details: null },
    }
  ...
  async runPlaywrightSmoke() {
    this.log('header', '🎭 Running Playwright Smoke')

    const command = process.env.CI
      ? 'npx playwright test --config=playwright.smoke.config.ts tests/smoke' // run whatever CI job requested
      : 'npm run test:smoke -- tests/smoke/catalog-cart-smoke.spec.ts'

    const result = await this.runCommand(
      command,
      'Playwright catalog → cart smoke',
    )
  ...
  generateReport() {
    ...
    const checks = [
      ...
      { name: 'Playwright', result: this.results.playwright },
